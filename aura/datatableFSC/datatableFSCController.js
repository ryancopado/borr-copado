...
searchTable : function(cmp,event,helper) {
        var allRecords = cmp.get("v.saveMydata");
        var searchFilter = event.getSource().get("v.value").toUpperCase();
        var tempArray = [];
    	var column01APIName=cmp.get("v.column01_fieldName");
    	var column02APIName=cmp.get("v.column02_fieldName");
        var i;
        for(i=0; i < allRecords.length; i++){
            if((allRecords[i][column01APIName] && allRecords[i][column01APIName].toUpperCase().indexOf(searchFilter) != -1)||
              (allRecords[i][column02APIName] && allRecords[i][column02APIName].toUpperCase().indexOf(searchFilter) != -1))
            {
                tempArray.push(allRecords[i]);
            }
        }
        cmp.set("v.mydata",tempArray);
    },
